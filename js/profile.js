...
function getUserId(raw) {
    let id = raw.trim();
    if (!id) return null;
    // If user already typed @s.whatsapp.net, don't duplicate
    if (!id.includes('@')) {
        id += '@s.whatsapp.net';
    }
    return id;
...
window.onload = function() {
    ...
    if (savedId) {
        // Show the plain number in input field (without @s.whatsapp.net)
...
async function loadProfile() {
    ...
    try {
        ...
        if (profile.error) {
            return content.innerHTML = `<p style="color:var(--danger);">${profile.error}</p>`;
        }
        ...
        // Render profile UI (same as before)
        content.innerHTML = `
