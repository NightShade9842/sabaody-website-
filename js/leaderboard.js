async function loadLB(type) {
    ...
    if (!data.length) {
        content.innerHTML = '<p style="color:var(--text2);">No data.</p>';
        return;
    }
    ...
    data.forEach((r, i) => {
        ...
        if (type === 'level') html += `<td>${r.pirate_name}</td><td>${r.level}</td><td>${formatNumber(r.xp)}</td>`;
        if (type === 'richest') html += `<td>${r.pirate_name}</td><td>${formatNumber(r.beli)}฿</td><td>${formatNumber(r.bank)}฿</td><td>${formatNumber(r.total)}฿</td>`;
        if (type === 'legendary') html += `<td>${r.pirate_name}</td><td>${r.count}</td>`;
